...
## Token authoring rules (Phase 2)

### Source of truth
- Figma variable exports live in `packages/tokens/src/figma/` — never edit these manually
- `packages/tokens/src/tokens.json` is generated by running `pnpm --filter @collective-discovery/tokens convert`
- After any Figma re-export, run `pnpm --filter @collective-discovery/tokens convert:build`

### Colour palette
- Primary brand: Charade palette (dark blue-grey, #262F39 source)
- Neutral/surface: Pale Snow palette (#F0F3F8 source)
- Status colours: error=#CE2029, success (Green palette, #54C603 source), warning=#FDEE00 (Aureolin), info=#007FFF (Azure)
- Font families: Manrope (primary headings), Inter (secondary body)

### Token tiers — never skip
- Primitive → Semantic → Component (never primitive → component directly)
- Semantic colour roles to use in components: bg-default, bg-subtle, bg-overlay, text-default, text-subtle, text-placeholder, text-inverse, text-on-accent, border-default, border-subtle, border-strong, border-focus
- Feedback states: error-default/subtle/text, success-default/subtle/text, warning-default/subtle/text, info-default/subtle/text

### Component token reference
- Button heights: xs=30, sm=36, md=42, lg=50, xl=60px
- Button compact heights: xs=22, sm=28, md=30, lg=34, xl=44px
- Button border-radius: 4px (border-radius-sm)
- Modal border-radius: 8px (border-radius-md)
- Card and modal background: bg-default (#F0F3F8)
- Overlay background: bg-overlay (#373E47)

### When updating tokens
1. Designer exports updated variables from Figma as JSON
2. Replace the relevant file in `packages/tokens/src/figma/`
3. Run `pnpm --filter @collective-discovery/tokens convert:build`
4. Commit both the figma source file and the regenerated tokens.json
5. CI rebuilds the Style Dictionary outputs automatically
